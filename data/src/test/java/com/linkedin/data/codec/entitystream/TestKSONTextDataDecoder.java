...
public class TestKSONTextDataDecoder
{
  private final KSONDataCodec KSON_TEXT_CODEC = new KSONDataCodec(false);
  ...
  {
    byte[] bytes = TestUtil.dataComplexToBytes(KSON_TEXT_CODEC, dataComplex);
    DataComplex decodedDataComplex = decode(bytes);
    assertEquals(TestUtil.dataComplexToBytes(KSON_TEXT_CODEC, decodedDataComplex), bytes);
  }
  ...
  public void testNumbers(Object number) throws Exception
  {
    ...
    byte[] bytes = TestUtil.dataComplexToBytes(KSON_TEXT_CODEC, dataMap);
  ...
  public void testInvalidMap() throws Exception
  {
    ...
    dataList.add(2);
    dataList.add(4);
    byte[] bytes = TestUtil.dataComplexToBytes(KSON_TEXT_CODEC, dataList);
    decode(bytes);
  ...
  {
    KSONDataDecoder<DataComplex> decoder = new KSONDataDecoder<>(false);
    return decode(bytes, decoder);
  }

  private static void decodeMap(byte[] bytes) throws Exception
  {
    KSONDataDecoder<DataMap> decoder = new KSONDataDecoder<>(false, false, null);
    decode(bytes, decoder);
  }

  private static void decodeList(byte[] bytes) throws Exception
  {
    KSONDataDecoder<DataList> decoder = new KSONDataDecoder<>(false, true, null);
    decode(bytes, decoder);
  }

...
[examples/quickstart](/linkedin/rest.li/tree/master/examples/quickstart)
...
[Gradle Build Integration](Gradle-build-integration). You will need
...
1.  Define data schema. Rest.li uses an Avro-like format known as
    \_[Pegasus Data
    Schema](/linkedin/rest.li/wiki/DATA-Data-Schema-and-Templates_) to
...
Templates](/linkedin/rest.li/wiki/DATA-Data-Schema-and-Templates) for
...
#### file: example-standalone-app/api/build.gradle
    ...
    <code>apply plugin: 'pegasus'</code>
...
#### file: example-standalone-app/server/src/main/java/com/example/fortune/impl/FortunesResource.java
    ...
    <code language="java">package com.example.fortune.impl;
...
Guide](/linkedin/rest.li/wiki/Rest.li-User-Guide) for more details about
...
return [ParSeq](/linkedin/parseq) `Promise` and `Task` classes so that
we do not block while performing IO operations.
...
Servlet
Containers](/linkedin/rest.li/wiki/Rest.li-with-Servlet-Containers).
...
#### file: example-standalone-app/server/gradle.properties
    ...
    <code>rest.model.compatibility=ignore</code>

This disables some [compatibility
checks](/linkedin/rest.li/wiki/Gradle-build-integration#compatibility)
...
command:
    ...
    <code>$ gradle startFortunesServer</code>
...
that needs to be passed in the request and builds a `Request`
    ...
    <code language="java">FortunesRequestBuilders fortunesBuilders = new FortunesRequestBuilders();
...
to send the request to the
    ...
    <code language="java">RestClient restClient = new RestClient(r2Client, "http://localhost:8080/");
...
interface to retrieve the results, like
    ...
    <code language="java">String message = response.getEntity().getFortune();
...
#### file: example-standalone-app/client/src/main/java/com/example/fortune/RestLiFortunesClient.java
    ...
    <code language="java">package com.example.fortune;
...
Then in another window, run:
    ...
    <code>$ gradle startFortunesClient</code>
...
Guide](/linkedin/rest.li/wiki/Rest.li-User-Guide).

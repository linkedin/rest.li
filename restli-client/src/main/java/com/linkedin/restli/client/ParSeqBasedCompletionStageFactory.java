...
public class ParSeqBasedCompletionStageFactory<T>
{
  ...
  /**
   * Convenient method to create a {@link ParSeqBasedCompletionStage} with a {@link Task} has not started.
   *
   * Will build {@link ParSeqBasedCompletionStage} by using a {@link Task} that has not started running, which will
   *    be run by the engine provided in this factory.
   *
   *
   ...
   *         :If {@link Task} produces an exception, this method will have the very exception, while {@link ##buildStageFromCompletionStage(CompletionStage)}
   *         is wrapping that with {@link java.util.concurrent.CompletionException}
   *

...
public abstract class AbstractCompressor implements StreamingCompressor
{
  ...
  {
    StreamingDeflater deflater = createDeflater(input);
    return EntityStreams.newEntityStream(deflater);
  }

  abstract protected StreamingInflater createInflater(EntityStream underlying);
  abstract protected StreamingDeflater createDeflater(EntityStream underlying);
}

...
public class ArgumentUtils
{
                                          ...
                                          boolean validateKey)
  {
    ...
    if(validateKey)
    {
      ...
      ValidationResult result = ValidateDataAgainstSchema.validate(underlyingValue, schema,
          new ValidationOptions(RequiredMode.CAN_BE_ABSENT_IF_HAS_DEFAULT, CoercionMode.OFF), validator);
      if(!result.isValid())
      {
        throw new RoutingException(String.format("Key with value '%s' is invalid, reason: %s",
            underlyingValue, result.getMessages()), HttpStatus.S_400_BAD_REQUEST.getCode());
      }
    }

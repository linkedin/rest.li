 ...
 * @author kparikh
 */
public interface AssociationResourceTask<V extends RecordTemplate> extends
    BaseResource,
    KeyValueResource<CompoundKey, V>
{
   ...
   */
  Task<Map<CompoundKey, V>> batchGet(Set<CompoundKey> ids);
   ...
   */
  Task<UpdateResponse> update(CompoundKey key, V entity);
   ...
   * @return the status or null if for a default, 204 No Content response
   */
  Task<UpdateResponse> update(CompoundKey key, PatchRequest<V> patch);
   ...
   * @return the status or null if for a default, 204 No Content response
   */
  Task<UpdateResponse> delete(CompoundKey key);
   ...
   */
  Task<BatchUpdateResult<CompoundKey, V>> batchUpdate(BatchUpdateRequest<CompoundKey, V> entities);
   ...
   * @return - update results
   */
  Task<BatchUpdateResult<CompoundKey, V>> batchUpdate(BatchPatchRequest<CompoundKey, V> patches);
   ...
   */
  Task<BatchUpdateResult<CompoundKey, V>> batchDelete(BatchDeleteRequest<CompoundKey, V> ids);
   ...
   */
  Task<List<V>> getAll(PagingContext pagingContext);
}

...
public interface RestLiDebugRequestHandler
{
   ...
   * @param attachmentReader The reader representing all incoming request level attachments.
   ...
   *
   * Note that since interface is based on RestRequest/RestResponse, response attachments are not possible. Which means that
   * when consumers of this API create a {@link com.linkedin.restli.server.RequestExecutionCallback} to pass to
   * handleRequest(), any response attachments need to be aborted. This applies to both
   * {@link RequestExecutionCallback#onSuccess(Object, RequestExecutionReport, RestLiResponseAttachments)} and
   * {@link RequestExecutionCallback#onError(Throwable, RequestExecutionReport, RestLiAttachmentReader, RestLiResponseAttachments)}.
   *
   * As a part of of handling the request as a regular request, this implementation will absorb any incoming request attachments provided,
   * unless there is an error. If this happens, it is the responsibility of the implementation of the
   * {@link com.linkedin.restli.server.RequestExecutionCallback} to absorb any request attachments and drop them on the ground
   * in {@link RequestExecutionCallback#onError(Throwable, RequestExecutionReport, RestLiAttachmentReader, RestLiResponseAttachments)}.
   */

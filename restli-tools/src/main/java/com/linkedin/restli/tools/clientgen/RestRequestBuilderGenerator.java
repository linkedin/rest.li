...
public class RestRequestBuilderGenerator
{
      ...
      throws IOException
  {
    ...
    RestRequestBuilderGenerator.run(resolverPath,
                                    ...
                                    args[0],
                                    sources,
                                    generateLowercasePath == null ? false : Boolean.parseBoolean(generateLowercasePath));
  }
      ...
      throws IOException
  {
    ...
    final JavaRequestBuilderGenerator generator = new JavaRequestBuilderGenerator(
            resolverPath, defaultPackage, generateDataTemplates, version, deprecatedByVersion, rootPath, generateLowercasePath);
    final ClassLoader classLoader = JavaCodeUtil.classLoaderFromResolverPath(resolverPath);

    final RestSpecParser.ParseResult parseResult = parser.parseSources(sources, generateLowercasePath);

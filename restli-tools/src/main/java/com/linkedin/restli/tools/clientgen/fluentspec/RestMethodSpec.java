...
public class RestMethodSpec
{
  ...
  public Set<ProjectionParameterSpec> getSupportedProjectionParams()
  {
    switch (ResourceMethod.fromString(_schema.getMethod()))
    {
      case GET:
      case BATCH_GET:
        return Collections.singleton(new ProjectionParameterSpec(RestConstants.FIELDS_PARAM, SpecUtils.FIELDS_MASK_METHOD_NAME, _root.getEntityClass()));
      case CREATE:
      case BATCH_CREATE:
      case PARTIAL_UPDATE:
      case BATCH_PARTIAL_UPDATE:
      ...
      case GET_ALL:
        ...
        collectionParts.add(new ProjectionParameterSpec(RestConstants.FIELDS_PARAM, SpecUtils.FIELDS_MASK_METHOD_NAME, _root.getEntityClass()));
        if (_schema.getMetadata() != null)
        {
          collectionParts.add(new ProjectionParameterSpec(RestConstants.METADATA_FIELDS_PARAM, SpecUtils.METADATA_MASK_METHOD_NAME, _root.classToTemplateSpec(_schema.getMetadata().getType())));
        }

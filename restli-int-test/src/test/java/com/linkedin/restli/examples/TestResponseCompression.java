...
public class TestResponseCompression extends RestLiIntegrationTest
{
  ...
  public void initClass() throws Exception
  {
    ...
    super.init(Arrays.asList(new TestHelperFilter()), null, "snappy,gzip", new CompressionConfig(10000));
  }
  ...
  private Object[][] requestData()
  {
    ...
    return new Object[][] {
        ...
        {null, RestliRequestOptions.DEFAULT_OPTIONS, largeIdCount, SHOULD_BE_PRESENT, null, true},
        {null, RestliRequestOptions.DEFAULT_OPTIONS, smallIdCount, SHOULD_BE_PRESENT, null, false},

        // Override the default threshold and cause small responses to be compressed
        {new CompressionConfig(tiny), RestliRequestOptions.DEFAULT_OPTIONS, largeIdCount, SHOULD_BE_PRESENT, tiny.toString(), true},
        {new CompressionConfig(tiny), RestliRequestOptions.DEFAULT_OPTIONS, smallIdCount, SHOULD_BE_PRESENT, tiny.toString(), true},

        // Override the default threshold and cause large responses to be NOT compressed
        {new CompressionConfig(huge), RestliRequestOptions.DEFAULT_OPTIONS, largeIdCount, SHOULD_BE_PRESENT, huge.toString(), false},
        {new CompressionConfig(huge), RestliRequestOptions.DEFAULT_OPTIONS, smallIdCount, SHOULD_BE_PRESENT, huge.toString(), false},

        // Force on/off using RestliRequestOptions
        {null, forceOnOption, largeIdCount, SHOULD_BE_PRESENT, zero.toString(), true},
        {null, forceOnOption, smallIdCount, SHOULD_BE_PRESENT, zero.toString(), true},
        {new CompressionConfig(huge), forceOnOption, smallIdCount, SHOULD_BE_PRESENT, zero.toString(), true},
        {null, forceOffOption, largeIdCount, SHOULD_NOT_BE_PRESENT, null, false},
        {null, forceOffOption, smallIdCount, SHOULD_NOT_BE_PRESENT, null, false},
        {new CompressionConfig(huge), forceOffOption, largeIdCount, SHOULD_NOT_BE_PRESENT, null, false},

        // Force on/off using ResponseCompressionConfig
        {new CompressionConfig(0), RestliRequestOptions.DEFAULT_OPTIONS, largeIdCount, SHOULD_BE_PRESENT, zero.toString(), true},
        {new CompressionConfig(0), RestliRequestOptions.DEFAULT_OPTIONS, smallIdCount, SHOULD_BE_PRESENT, zero.toString(), true},
        {new CompressionConfig(Integer.MAX_VALUE), RestliRequestOptions.DEFAULT_OPTIONS, largeIdCount, SHOULD_BE_PRESENT, max.toString(), false},
        {new CompressionConfig(Integer.MAX_VALUE), RestliRequestOptions.DEFAULT_OPTIONS, smallIdCount, SHOULD_BE_PRESENT, max.toString(), false},

        // RestliRequestOptions takes precedence over ResponseCompressionConfig
        {new CompressionConfig(0), forceOffOption, largeIdCount, SHOULD_NOT_BE_PRESENT, null, false},
        {new CompressionConfig(Integer.MAX_VALUE), forceOnOption, smallIdCount, SHOULD_BE_PRESENT, zero.toString(), true}
    };
  }

  @Test(dataProvider = "requestData")
  public void testResponseCompression(CompressionConfig responseCompressionConfig, RestliRequestOptions restliRequestOptions, int idCount,
                                      String expectedAcceptEncoding, String expectedCompressionThreshold, boolean responseShouldBeCompressed)
      throws RemoteInvocationException, CloneNotSupportedException
  {
    ...
    {
      Assert.assertEquals(response.getHeader(HttpConstants.CONTENT_ENCODING), EncodingType.GZIP.getHttpName());
    }
    else
    {
      Assert.assertNull(response.getHeader(HttpConstants.CONTENT_ENCODING));
    }
  }
}
